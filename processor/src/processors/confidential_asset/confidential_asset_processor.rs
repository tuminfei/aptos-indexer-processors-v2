...
impl ProcessorTrait for ConfidentialAssetProcessor {
    ...
    async fn run_processor(&self) -> Result<()> {
        ...
        let processor_config = match &self.config.processor_config {
            ProcessorConfig::ConfidentialAssetProcessor(processor_config) => processor_config,
        ...
        let storer = ConfidentialAssetStorer::new(
            self.db_pool.clone(),
            processor_config.per_table_chunk_sizes.clone(),
        );
