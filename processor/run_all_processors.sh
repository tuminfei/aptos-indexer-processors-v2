...
nohup ./target/release/processor -c config_fungible_asset.yaml > fungible_asset_processor.log 2>&1 &
FUNGIBLE_PID=$!
...
nohup ./target/release/processor -c config_user_transaction.yaml > user_transaction_processor.log 2>&1 &
USER_TXN_PID=$!
...
nohup ./target/release/processor -c config_account_transactions.yaml > account_transactions_processor.log 2>&1 &
ACCOUNT_TXN_PID=$!
echo "account_transactions_processor started with PID: $ACCOUNT_TXN_PID"

# 运行 default_processor
echo "Starting default_processor..."
nohup ./target/release/processor -c config_default.yaml > default_processor.log 2>&1 &
DEFAULT_PID=$!
